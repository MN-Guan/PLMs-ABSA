...
class PromptDataset(Dataset):
    def __init__(self, pre_model, f_name, tokenizer, max_seq_len, data_name, keyword_dict):
        if pre_model == 'gpt2':
            self.Z = 'mask'
        else:
            self.Z = '<mask>'
    ...
    def _get_data(self):
        ...
        for i in range(0, len(lines), 3):
            keyword = keyword_dict[data_name]
            id += 1
...
if __name__ == '__main__':
    if not os.path.exists('log'):
        os.mkdir('log')
    if not os.path.exists('log/find_prompt'):
        os.mkdir('log/find_prompt')
    pre_train_map = {
        'bart-base': {
    ...
    keyword_dict_bert = {'restaurant': 'expression', 'laptop': 'eyeshot', 'twitter': 'follow-up', 'res15': 'vista', 'res16': 'vista'}
    ...
    for data_name in keyword_dict.keys():
    #for data_name in ['laptop']:
        file = os.path.basename(sys.argv[0])  # 当前文件名名称
        logger = logging.getLogger()
        logger.setLevel(logging.INFO)
        logger.addHandler(logging.StreamHandler(sys.stdout))
        log_file = 'log/find_prompt/{}_{}_{}_{}.log'.format(file.split('.')[0], pretrained_model, data_name, time.strftime("%y%m%d-%H:%M", time.localtime()))
        logger.addHandler(logging.FileHandler(log_file))

        best_score = 0
        logger.info('>> DATASET:{} START FIND PROMPT TEMPLATE'.format(data_name))
        dataset = PromptDataset(pretrained_model, data_files[data_name]['train'], tokenizer, max_seq_len, data_name, keyword_dict)
