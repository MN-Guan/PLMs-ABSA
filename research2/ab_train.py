...
from n_data_utils import build_tokenizer, build_embedding_matrix, Tokenizer4Bert, ABSADataset
from models.ablation import BERT_AB_PROMPT, MODEL_BASE
from models.n_mpcg_altc import CrossEntropyLoss_LSR
...
class Instructor:
    def __init__(self, opt):
        self.opt = opt
        tokenizer = Tokenizer4Bert(opt.max_seq_len, pre_train_map[opt.pretrained_bert_name]['tokenizer_file'])
        if opt.pretrained_bert_name == 'bart-base':
            bert = BartModel.from_pretrained(pre_train_map[opt.pretrained_bert_name]['pre_model_file'])
        elif opt.pretrained_bert_name == 'roberta-base':
            bert = RobertaModel.from_pretrained(pre_train_map[opt.pretrained_bert_name]['pre_model_file'])
        elif opt.pretrained_bert_name == 'gpt2':
            bert = GPT2Model.from_pretrained(pre_train_map[opt.pretrained_bert_name]['pre_model_file'])
        else:
            bert = BertModel.from_pretrained(pre_train_map[opt.pretrained_bert_name]['pre_model_file'])
    ...
    def _reset_params(self):
        ...
        for child in self.model.children():  # model的大框架
            if type(child) != BertModel and type(child) != BartModel \
                    and type(child) != RobertaModel and type(child) != GPT2Model:  # skip bert params
                print(type(child))
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                outputs = torch.log(self.model(inputs))
    ...
    def _write_detail(self, test_acc, test_f1, acc_detail, f1_detail):
        result_file = 'log/{}/{}/result_{}.csv'.format(self.opt.log_date, self.opt.dataset, self.opt.dataset)
        if not os.path.exists(result_file):
            ...
            if self.opt.model_name == 'bert_mpcg_altc':
                title.append('mpc')
...
def main():
    ...
    # 模型名称
    parser.add_argument('--model_name', default='bert_ab_prompt', type=str, help='bert_*mpcg_altc,*:b(mpc=2),'
                                                                                  't(mpc=3),f(mpc=4),fi(mpc=5)')
    # 数据集
    parser.add_argument('--dataset', default='laptop', type=str, help='twitter, restaurant, laptop, res15, res16')
    # 优化函数
    ...
    parser.add_argument('--prompt_type', default=1, type=int)
    parser.add_argument('--log_date', default='0', type=str)
    ...
    parser.add_argument('--local_gate', default='False', type='boolean', help='局部识别是否需要门控')
    ...
    model_classes = {
        # 模型调用
        'bert_ab_prompt': BERT_AB_PROMPT,  # w/o prompt
    ...
    input_colses = {
        'bert_ab_prompt': ['bert_indices', 'segments_indices'],  # w/o prompt
        'model_base': ['bert_indices']  # w/o prompt
    }
    ...
    opt.inputs_cols = input_colses[opt.model_name]  # 对应模型所需要的栏
    ...
    if not os.path.exists('log/{}/{}'.format(opt.log_date, opt.dataset)):
        os.mkdir('log/{}/{}'.format(opt.log_date, opt.dataset))
    log_file = 'log/{}/{}/class{}_{}-{}.log'.format(opt.log_date, opt.dataset, opt.polarities_dim, opt.model_name,
